...
public class AdminMenuHelper {
	...
	public static boolean hasAccess(String featureId) {
		...
		if (userDetails != null) {
			...
			AdminMenuHelper adminHelper = new AdminMenuHelper();
			if (adminHelper.isFeaturePresent(userDetails.getAuthorities(), featureId)) hasAccess = true;
		}
		return hasAccess;
	}
	 ...
	 */
	private boolean isFeaturePresent(Collection<? extends GrantedAuthority> grantAuthorities, String featureId) {
		boolean isFeaturePresent = false;
		SimpleGrantedAuthority featureAuthority = new SimpleGrantedAuthority(featureId);
		if (CollectionUtils.containsAny(grantAuthorities, Arrays.asList(featureAuthority))) {
